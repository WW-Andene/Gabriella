...
function shouldUpdate(model) {
  const turnsSinceUpdate = model.turnCount - (model._lastUpdateTurnCount ?? 0);
  if (turnsSinceUpdate < UPDATE_EVERY_N_TURNS) return false;
  const age = Date.now() - (model.lastUpdated || 0);
  if (age < MIN_COOLDOWN_MS) return false;
  return true;
}
