...
import { rolloutJudge, rolloutEnabled } from "./counterfactual.js";
...
export async function bestOfTwo({
  ...
  feltState,
  lastUserMessage,
  // Counterfactual rollout inputs — when provided AND pragmatics is
  // heavy, the rollout judge simulates the user's likely next turn
  // for each candidate and picks the trajectory that goes somewhere
  // real. Opt-in so baseline runs stay cheap.
  redis,
  conversation,       // full message list for the rollout simulator
  pragmaticWeight,
}) {
  ...
  // Heavy-moment rollout: simulate turn+2 for each candidate and pick
  // the trajectory that scores best on depth/warmth/authenticity/
  // momentum. Falls through to regular pickBest on failure.
  let rolloutPick = null;
  if (redis && conversation && rolloutEnabled(pragmaticWeight)) {
    try {
      rolloutPick = await rolloutJudge({
        redis,
        conversation,
        candidates: [primaryText, shadowText],
        pragmaticWeight,
      });
    } catch { rolloutPick = null; }
  }

  const winnerIdx = rolloutPick
    ? rolloutPick.chosenIndex
    : await pickBest({ lastUserMessage, candidates: [primaryText, shadowText], feltState });

  return {
    chosen:        winnerIdx === 0 ? primaryText : shadowText,
    chosenIndex:   winnerIdx,
    chosenBy:      rolloutPick ? "rollout" : "judge",
    primary:       primaryText,
    shadow:        shadowText,
    crossProvider: !!shadowFn,
    rollout:       rolloutPick || null,
  };
}
