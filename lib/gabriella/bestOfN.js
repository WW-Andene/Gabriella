...
// ─── Orchestrator ───────────────────────────────────────────────────────────
//
// Runs two speaker calls in parallel (primary + shadow with tuned
// params), then picks the winner via pickBest. The caller supplies a
// `speakFn(params, opts)` callback so this module stays decoupled
// from speaker.js's internals (which have heavy Fireworks + Groq
// routing). The callback is the EXACT same function the original
// speak() would have called, just invoked twice with different params.

export async function bestOfTwo({ speakFn, primaryParams, shadowParams, feltState, lastUserMessage }) {
  const [primary, shadow] = await Promise.allSettled([
    speakFn(primaryParams),
    speakFn(shadowParams),
  ]);

  const primaryText = primary.status === "fulfilled" ? primary.value : null;
  const shadowText  = shadow.status  === "fulfilled" ? shadow.value  : null;

  // Both failed — return null, caller handles.
  if (!primaryText && !shadowText) return { chosen: null, chosenIndex: 0, chosenBy: "failure" };
  // Only one succeeded — no judging needed.
