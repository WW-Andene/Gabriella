...
import { pickSeedExemplars } from "./seedExemplars.js";

const CACHE_TTL_MS = 60_000;
const MIN_QUALITY_POOL = 20;   // below this, we backfill from seeds
...
export async function findExemplars(redis, userId, currentMoment, {
  ...
  // ── Cold-start fallback ─────────────────────────────────────────────────
  // If the filtered training pool is below MIN_QUALITY_POOL (brand-new
  // deploy, or early sessions before the training_log has accumulated),
  // backfill with hand-curated seed exemplars. This is the crucial piece
  // that makes turn 1 through turn ~50 benefit from ICL on a fresh
  // deployment — without it, every new user gets zero-shot voice for
  // weeks.
  //
  // Also used when a real-log pool exists but didn't score anything
  // meaningful against the current moment: the seed pool covers failure
  // classes (phatic, meta, heavy, provocation) that a sparse real log
  // may not have examples of yet.
  if (chosen.length < k) {
    const needed = k - chosen.length;
    const seeds = pickSeedExemplars(currentMoment, { k: needed, minScore: 0.0 });
