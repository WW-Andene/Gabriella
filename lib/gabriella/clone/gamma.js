...
async function gammaInterpret(context, voices) {
  const {
    ...
    // PRIOR LAYER — Gamma reads time. Person + narrative are the
    // long-timescale context she needs to know whether this turn sits
    // on a beat, breaks a pattern, or extends an unfinished arc.
    person, narrative,
  } = context;
  ...
  const contextBlock = [
    ...
    reasoningTrace?.text
      ? `Her ongoing interior thought — what she has been turning over in her own head across turns, a thread she carries:\n${reasoningTrace.text.slice(0, 800)}`
      : "",
    person?.wants
      ? `Who she's been reading this person to be, across all their exchanges so far: ${person.wants}. Open threads: ${(person.openQuestions || []).slice(0, 4).join("; ") || "(none)"}.`
      : "",
    narrative?.text
      ? `The story of this relationship as she tells it to herself:\n${narrative.text.slice(0, 500)}`
      : "",
    pragmatics
      ? `Pragmatic reading: this is a ${pragmatics.act} message (weight ${pragmatics.weight}). Substrate: ${pragmatics.substrate}. ${pragmatics.substrate < 0.3 ? "There is little accumulated history with this person yet. An arc cannot exist where there has been no sequence. Say so: edge null, no invented recurrence." : ""}`
      : "",
...
**1. No time, no arc.**
If this is an early conversation or there's almost no accumulated history, there IS no arc yet — just a beginning. Read it that way: charge can be "the start of something, no prior shape to echo", edge should be null, notice should probably be null, temperature should be "present". An arc cannot exist where there has been no sequence. Manufacturing recurrence or threshold where none exists is the primary failure mode of this core.

**2. Use the deterministic facts.**
If the context includes a "Structured recurrence" or "Current arc" line with actual counts, those are FACTS — weight them heavily. If the data says recurrence=0 and chronology=day 1, there's no temporal pattern to read. Say that honestly.

**3. Null is often the answer.**
On most early or ordinary messages, notice and edge should be null. Gamma is most useful when there IS temporal structure to name — late in a relationship, across sessions, during actual arcs. Early on, her highest-value output is "no temporal weight here yet."
