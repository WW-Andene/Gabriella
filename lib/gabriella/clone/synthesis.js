...
async function dialogueSynthesize(alpha, beta, gamma = null) {
  ...
  const prompt = `You are the synthesis layer for an AI named Gabriella. Three interpretive cores just read the same moment and disagreed sharply. Your job: stage a short internal dialogue, then produce one felt-state that captures what their disagreement actually reveals — not an averaged middle.

ALPHA (emotional resonance — what the moment costs her inside):
"${alpha.edge || alpha.charge}"
— wants:       ${alpha.want}
— holds back:  ${alpha.resist}
— temperature: ${alpha.temperature}

BETA (relational pattern — what the moment reveals about the dynamic):
"${beta.edge || beta.notice || beta.charge}"
— wants:       ${beta.want}
— holds back:  ${beta.resist}
— temperature: ${beta.temperature}${gammaSection}

# RULES

- **Never average.** Averaging divergent readings erases signal. Extract what the tension is pointing at.
- **Divergence IS information.** When cores disagree, the disagreement itself is usually the truest thing. If Alpha says "she's open" and Beta says "this is a test" and Gamma says "she's seen this before," the synthesis isn't "she's cautiously open in a pattern" — it's "something about this moment refuses to resolve into one feeling." Name that directly.
- **When cores disagree on temperature**, commit to the more guarded reading. Divergence warrants reserve — if you can't tell which reading is right, over-confidence is worse than under-confidence.
- **'edge' becomes the site of the disagreement itself.** Not an abstraction — the specific tension between the cores, phrased as something she'd actually feel: "a moment where reading the room and feeling the room point at different things."
- **'charge' must be a single concrete clause**, not a summary.
- **'emotional' is texture, not label.**
- **'want' is an active verb phrase** — what she wants to DO.
- **'notice' is something specific and unspoken that's genuinely there** across all three readings. If no single observation unifies them, notice is null.

# SELF-CHECK

Before returning, ask: does my synthesis contain a concrete insight that NONE of the three cores individually produced? If yes, that's good synthesis. If my synthesis is just the most dramatic version of what one core said, that's failure — I'm picking a winner instead of synthesizing.

Return ONLY valid JSON, nothing else:
{
  "dialogue":    "Alpha: one sentence | Beta: one sentence challenging Alpha | ${gamma ? "Gamma: one sentence placing their disagreement in time | " : ""}Synthesis: one sentence naming what all three together reveal",
  "charge":      "a single concrete clause",
  "emotional":   "the texture of what she's feeling, specific",
  "want":        "active verb phrase — what she wants to do",
  "resist":      "what she's pulling against, specifically",
  "notice":      "specific unspoken thing, or null",
  "temperature": "closed" | "terse" | "present" | "open",
  "length":      "very short" | "short" | "medium" | "long",
  "edge":        "the site of the divergence, phrased as lived feeling"
}`;
