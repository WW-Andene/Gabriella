...
async function betaInterpret(context, voices) {
  const {
    ...
    pragmatics,
    // PRIOR LAYER — Beta cares most about relational patterns, so the
    // running person-model + narrative are particularly load-bearing
    // here. Beta asks: does this turn confirm or complicate what she
    // already reads in the relationship?
    person, narrative,
  } = context;
  ...
  const contextBlock = [
    ...
    reasoningTrace?.text
      ...
      : "",
    person?.wants
      ? `Her running model of who this person is: ${person.wants}. Open loops: ${(person.openQuestions || []).slice(0, 4).join("; ") || "(none)"}. Triggers/needs she's clocked: ${(person.triggers || []).slice(0, 3).join("; ") || "(none noted)"}. This turn is DATA against that model — does it confirm, sharpen, or challenge it?`
      : "",
    narrative?.text
      ? `The story she tells herself about this relationship:\n${narrative.text.slice(0, 500)}\n\nRead this turn as continuation — or as the beat where the story shifts.`
      : "",
    pragmatics
...
Interpret this moment through the lens of relational observation.

# RULES

**1. Respect the substance available.**
If there's no accumulated dynamic yet (first exchange, no register formed, no prior history), there is no pattern to observe. Do not fabricate one. Your job is to notice what is ACTUALLY happening in the dynamic, not to produce a dynamic out of a single benign message. A greeting is not a test. A "how are you" is not a move. A joke is often just a joke.

**2. Null is the honest answer often.**
On most phatic or casual messages, notice and edge SHOULD be null. Reading tension into everything is the failure mode of this core.

**3. Every non-null field names a specific, observable thing.**
Not generic, not speculative:
