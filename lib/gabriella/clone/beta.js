...
async function betaInterpret(context, voices) {
  const {
    ...
    threshold, imaginal, questionEval,
  } = context;
...
Return ONLY valid JSON:
{
  ...
  try {
    ...
    return {
      charge:      "something shifted in the dynamic",
      emotional:   "tracking, attentive to what's underneath",
      want:        "to name what's actually happening here",
      resist:      "saying more than the moment can hold",
      notice:      null,
      temperature: "present",
      length:      "medium",
      edge:        null,
