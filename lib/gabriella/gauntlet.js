...
async function runCheck(prompt) {
  try {
    const result = await pickClient().chat.completions.create({
      ...
      temperature: 0.1,
      max_tokens: 80,
      // OpenAI-compat JSON mode — supported by Groq and Fireworks. Forces
      // the model to emit parseable JSON, eliminating the regex-fence-strip
      // retry class. Cerebras honors it on most models; Gemini's OpenAI
      // shim accepts the field silently. On providers that ignore it, the
      // fall-through parse is still safe because the prompt already asks
