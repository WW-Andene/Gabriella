...
export async function runGauntlet(response, messages, withheld, questionEval, agenda, activeThreshold, opts = {}) {
  // Routing mode from turnShape — "full" (7 checks), "lean" (3
  // critical checks), "skip" (pass-through). Default "full" for
  // backwards compat with callers that haven't been updated.
  const mode = opts.mode || "full";

  // Explicit skip from turnShape: trusting user, low weight, no
  // provocation/correction. The calibration below is still honored
  // — we never skip when the response itself is suspect.
  if (mode === "skip") {
    return { pass: true, failures: [], mode: "skipped" };
  }
  ...
  if (response.trim().split(/\s+/).length <= 12) {
    return { pass: true, failures: [], mode: "short-circuit" };
  }
  ...
  if (messages.length < 6) {
    return { pass: true, failures: [], mode: "early" };
  }

  // "lean" runs only the 3 most critical checks — the ones that
  // catch the failures most likely to poison a trusting relationship:
  //   premature   (wrapping up what should stay open)
  //   exposed     (giving away a withheld item too soon)
  //   voiceDrift  (slipping out of her voice)
  // We drop compliant, abandoned, evasive, offTopic — these are more
  // relevant on heavy/testing turns than on lean ones.
  if (mode === "lean") {
    const [premature, exposed, voiceDrift] = await Promise.all([
