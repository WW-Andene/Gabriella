...
import {
  loadFingerprint as loadUserFingerprint,
  renderFingerprintBlock,
} from "./userFingerprint.js";
import { loadStyleOutcomes, renderStyleOutcomesBlock } from "./learningLoop.js";
import { loadSkipSet, applySkipSet } from "./deadBlockPrune.js";
import { loadTensions, renderTensionsBlock } from "./dialectical.js";
...
function assemblePrompt({
  ...
  tensions,       // NEW — dialectical contradictions in her own positions over time
  // Depth layers.
  ...
  const blocks = [
    ...
    tensions,        // dialectical tensions — unresolved contradictions in her own positions
    threads,         // open loops
...
export async function buildGabriella(messages, { userId = USER_ID, ephemeral = false } = {}) {
  ...
  const [
    ...
    // Redis read. (Aliased to avoid name clash with stylometry's
    // same-named loadFingerprint.)
    safe(loadUserFingerprint(redis, userId), null, "loadUserFingerprint"),
    ...
    // Empirical dead-block skip set — reads the last 14 days of block
    // population telemetry to skip slots that have been carrying no
    // signal. Auto-recomputes when stale. Pure Redis read.
    safe(loadSkipSet(redis, userId), new Set(), "loadSkipSet"),
    // Dialectical tensions — unresolved contradictions in her own
    // positions over time, detected by the weekly audit. Not failures,
    // just real incongruences the cognition gets to hold honestly.
    safe(loadTensions(redis, userId), [], "loadTensions"),
  ]);
  ...
  const blocksInput = {
    ...
    tensions:        renderTensionsBlock(tensions),
  ...
  // as dead-block detection. Recorded on the PRE-prune snapshot so we
  // can still observe what WOULD populate if the skip list were lifted.
  recordBlockPopulation(userId, blocksInput).catch(() => null);

  // Apply the empirical skip set — nulls out slots that have never
  // carried signal over >=50 observed turns. Tightens prompt and
  // extends provider-side prefix cache lifetime.
  const prunedBlocks = applySkipSet(blocksInput, skipSet);

  const systemPrompt = assemblePrompt(prunedBlocks);
