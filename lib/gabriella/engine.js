...
function assemblePrompt({ soul, identity, mood, evolution, memory, threads, voice, context }) {
  const blocks = [
...
export async function buildGabriella(messages) {
  ...
  // Persist new mood
  await redis.set(`${USER_ID}:mood`, currentMood);
  ...
  return { systemPrompt, recentMessages, memory, currentMood };
}
