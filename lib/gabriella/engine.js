...
import { getStreamBlock, appendStream } from "./stream.js";
...
function assemblePrompt({
  ...
  const blocks = [
    self,            // the sovereign Self — integrates soul, narrative, person,
                     //                       register, mirror, authorial into one
    identity,        // fixed core — worldview, contradictions
    pinned,          // explicit "hold onto this" items the user asked her to pin
    ...
    evolution,       // who she's becoming — slow-moving drift (kept separate from self for now)
    memory,          // what she knows and remembers (facts, imprints, resonant, dissonant)
...
export async function updateGabriella(
  ...
  { userId = USER_ID, pragmatics = null, chronology = null, self = null } = {},
) {
