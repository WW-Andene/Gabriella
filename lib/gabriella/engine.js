...
export async function updateGabriella(
  ...
  // Stylometry: record this response and refresh the fingerprint
  // opportunistically. The prompt block will pick up the updated
  // fingerprint on the next turn. Fire-and-forget; never blocks.
  if (fullReply && fullReply.trim().length >= 30) {
    recordResponse(redis, userId, fullReply).catch(() => null);
    // Only refresh the aggregated fingerprint every ~5 turns to save
    // Redis writes — the stats are a rolling window anyway.
