...
export async function buildGabriella(messages, { userId = USER_ID, ephemeral = false } = {}) {
  ...
  // User mood trajectory — text-feature signal across their last ~8
  // messages. Separate from her read of them; observational.
  const userMoodBlock = await safe(
    renderUserMoodBlock(redis, userId),
    "",
    "renderUserMoodBlock",
  );

  // User identity hooks — name + pronouns extracted from prior user
  // messages and stored as stable facts. Loaded every turn so the
  // speaker can reference naturally.
  const identityHooks = await safe(
    loadIdentity(redis, userId),
    {},
    "loadIdentity",
  );
