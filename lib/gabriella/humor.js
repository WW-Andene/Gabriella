...
// Even when a wit pattern technically matches, suppress if the contextual
// weight signals genuine vulnerability. The pragmatic-weight gate is the
// existing line — if the moment scores >= 0.6 (substantive), wit is off.
...
export function shouldSuppressWit({ pragmaticWeight, feltState }) {
  ...
  if (feltState?._silence) return true;   // silence policy fired
  if (feltState?.temperature === "open") return true;  // open moments warrant earnestness
  return false;
}
