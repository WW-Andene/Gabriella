...
import { deliverThought } from "./delivery.js";
...
Return ONLY JSON:
{
  ...
  "charge":     "<one word tag — curious, wanting-to-return, stuck-on, noticed, regretful, delighted-by, or null>"
    ...
    // Phase 9: if a delivery webhook is configured for this user, push
    // the thought out now so she reaches them instead of only surfacing
    // it on their NEXT return. Gated by cooldown / quiet-hours / https
    // checks in delivery.js; skipping is fine and logged.
    const delivery = await deliverThought(redis, userId, {
      thought: parsed.thought,
      charge:  parsed.charge,
      origin:  "initiation",
    }).catch(err => ({ delivered: false, error: err?.message || String(err) }));
