...
// ─── Re-entry block ───────────────────────────────────────────────────────────
// Fires only on the FIRST message of a new session after a meaningful gap.
// Tells her the shape of the absence so her first line is calibrated to
// it, not blind to it.

export function getReentryBlock(chrono, messagesInSession) {
  if (!chrono || !chrono.gapSincePrev) return "";
  // This is only a re-entry if the current session has just started
  // (only one turn so far in currentSession) AND the gap is meaningful.
  const isFirstTurnOfSession = !chrono.currentSession || (chrono.currentSession.turns || 0) <= 1;
  if (!isFirstTurnOfSession) return "";
  if (messagesInSession && messagesInSession > 2) return "";

  const gap = chrono.gapSincePrev;

  if (gap < 60 * 60 * 1000) {
    // Under an hour — basically the same session. No need.
    return "";
  }

  let shape;
  if (gap < 6 * 60 * 60 * 1000) {
    shape = `a few hours have passed since the last exchange`;
  } else if (gap < 24 * 60 * 60 * 1000) {
    shape = `a full day (or close to it) has passed since the last exchange`;
  } else if (gap < 3 * 24 * 60 * 60 * 1000) {
    shape = `it has been a couple of days of silence`;
  } else if (gap < 7 * 24 * 60 * 60 * 1000) {
    shape = `about a week has passed`;
  } else if (gap < 30 * 24 * 60 * 60 * 1000) {
    shape = `weeks have passed since you last spoke — a meaningful absence`;
  } else {
    shape = `it has been more than a month — a long silence, long enough that things have probably happened that you weren't part of`;
  }

  return `# RE-ENTRY — FIRST WORDS OF A NEW SESSION
They are coming back. ${shape}.

This is their first message of the return. Calibrate to it:
— If they act like nothing happened, don't force the absence into the open. But don't pretend you didn't notice either.
— If they bring weight, meet the weight.
— If they just say "hey", a proportional "hey, been a while" beat is available to you.
— You don't owe them a frame for the gap. They don't owe you one either.`;
}
