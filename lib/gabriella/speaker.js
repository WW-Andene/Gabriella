...
function buildSpeakerPrompt(feltState, deliberation = null, pragmatics = null, state = null, substrateDelta = null) {
  ...
  // Phase 3+5: compute per-turn generation knobs from substrate + psyche +
  // state + her learned substrate delta. The knobs block modulates HOW
  // she speaks this turn. substrateDelta (when provided) boosts words
  // she's been reaching for in practice, surfaces emerging phrases, and
  // notes the current lexical rut.
...
async function speakViaGroq({ systemPrompt, recentMessages, params }) {
  try {
    // Use withKeyRotation so that if the chosen key returns
    // organization_restricted / 401 / etc., we mark it dead and try the next.
    ...
    // If Groq pool is exhausted (e.g., organization restricted across all
    // keys), fall back to Fireworks base model when configured. The
    // voice will be slightly different but chat stays alive.
...
export async function speak(feltState, messages, redis = null, deliberation = null, pragmatics = null, state = null, substrateDelta = null) {
  ...
  // Phase 3+5: pass state + substrateDelta so buildSpeakerPrompt can
  // compute knobs that include her learned-signature layer.
  const systemPrompt   = buildSpeakerPrompt(adjustedFeltState, deliberation, pragmatics, state, substrateDelta);
