...
import { adaptParams, explainAdaptation } from "./adaptiveParams.js";
...
export async function speak(feltState, messages, redis = null, deliberation = null, pragmatics = null, state = null, substrateDelta = null, userId = null, selfRead = null) {
  ...
  const withBoost      = tokenBoost > 0
    ? { ...baseParams, max_tokens: baseParams.max_tokens + tokenBoost }
    : { ...baseParams };

  // Signal-coupled adaptation — nudge temperature/top_p/max_tokens
  // from per-user signals that are already in Redis. Delta-bounded
  // so the mood-driven prior stays dominant.
  let adaptSignals = null;
  if (redis && userId) {
    try {
      const [styleOutcomes, userRead] = await Promise.all([
        loadStyleOutcomes(redis, userId).catch(() => null),
        loadUserRead(redis, userId).catch(() => null),
      ]);
      adaptSignals = {
        styleOutcomes,
        stance:         userRead?.stance || "unknown",
        heatmap:        userRead?.heatmap || null,
        rolloutConfidence: null,  // populated later if rollout fires
      };
    } catch { /* silent */ }
  }
  const adapted = adaptSignals
    ? adaptParams(withBoost, adaptSignals)
    : withBoost;
  const params = { ...adapted, _feltState: adjustedFeltState, _adaptation: explainAdaptation(withBoost, adapted) };
  ...
  const result = await bestOfTwo({
    ...
    feltState:     adjustedFeltState,
    lastUserMessage,
    // Counterfactual rollout — only fires on heavy moments
    // (pragmatics.weight >= 0.5). Safe to pass unconditionally;
    // rolloutEnabled gates the extra calls internally.
    redis,
    conversation:     messages,
    pragmaticWeight:  pragmatics?.weight ?? null,
  });
