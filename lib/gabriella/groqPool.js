...
const MODEL_MAP = {
  cerebras: {
    // Fast tier maps cleanly.
    "llama-3.1-8b-instant": "llama3.1-8b",
    // Premium tier: Cerebras's free/public catalog doesn't currently
    // serve llama-4-scout-17b-16e-instruct — returns 404 model_not_found.
    // Fall back to their largest public Llama (3.3-70b), which is a
    // reasonable quality match and definitely available.
    "meta-llama/llama-4-scout-17b-16e-instruct": "llama-3.3-70b",
  },
  gemini: {
    // Gemini's OpenAI-compat endpoint expects Gemini model ids.
    // Fast tier → flash, premium tier → flash too (pro is rate-limited
    // on free tier; flash is plenty for gauntlet / metacognition / cores).
    "llama-3.1-8b-instant":                       "gemini-2.5-flash",
    "meta-llama/llama-4-scout-17b-16e-instruct":  "gemini-2.5-flash",
  },
};
...
// options:
//   providers: string[] — restrict selection to given providers
//   excludeProviders: string[] — skip these providers
export function pickClient(options = {}) {
  mustPool();
  const alive = aliveIndices();
  const filtered = alive.filter(i => {
    const p = CLIENTS[i].provider;
    if (options.providers && !options.providers.includes(p)) return false;
    if (options.excludeProviders && options.excludeProviders.includes(p)) return false;
    return true;
  });

  // If the filter left nothing alive, fall through to any live client —
