...
function computePauses(fragments, { comfort, register, state }) {
  ...
  for (let i = 0; i < fragments.length - 1; i++) {
    ...
    // Base: 2s + ~20ms per char of the NEXT fragment (longer aside → longer
    // pause before she produces it).
    let base = 2000 + Math.min(2500, nextLen * 20);

    // Comfort shaves pauses — more at-ease = less hesitation.
    base -= (comfort - 0.5) * 1000;   // ±500ms

    // Register shaves — casual is quicker between sends.
    if (register === "textedCasual") base *= 0.85;
    if (register === "textedTired")  base *= 1.1;
