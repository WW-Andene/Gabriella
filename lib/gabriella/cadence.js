...
const MAX_PRE_DELAY_MS = 5000;
const MIN_PRE_DELAY_MS = 150;
...
export function computeCadence({
  ...
  if (act === "phatic" || weight < 0.2) {
    base = 200 + Math.random() * 200;      // 200-400ms
  } else if (weight < 0.45) {
    base = 500 + Math.random() * 400;      // 500-900ms
  } else if (weight < 0.7) {
    base = 900 + Math.random() * 600;      // 900-1500ms
  } else {
    base = 1400 + Math.random() * 1200;    // 1400-2600ms
  }
  ...
  const lengthFloor = Math.min(800, responseLength * 2);
