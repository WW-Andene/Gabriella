...
export function parseMonologue(raw) {
  if (!raw || typeof raw !== "string") {
    return { innerThought: null, response: "", uncertain: null, truncated: false };
  }

  const openIdx  = raw.indexOf("<think>");
  const closeIdx = raw.indexOf("</think>");

  // CASE 1 — truncated while inside <think>: open tag present, no close tag.
  // This happens when the model hits max_tokens during the hidden
  // monologue. The entire output is inner thought; visible response is
  // empty. Flag truncated=true so the caller can retry with more budget
  // instead of streaming "<think>..." to the user.
  if (openIdx !== -1 && closeIdx === -1) {
    const thoughtPart = raw.slice(openIdx + 7).trim();
    return {
      innerThought: thoughtPart || null,
      response:     "",
      uncertain:    null,
      truncated:    true,
    };
  ...
  // Unclosed <uncertain> is left as-is — it's more likely to be a
  // legitimate literal token in the user's reply than a truncated tag.

  if (openIdx === -1 || closeIdx === -1) {
    // No think block at all — treat stripped text as visible response.
    // Defensive: strip any orphan <think> or </think> fragments that might
    // slip through (e.g., model emitted partial tags).
    const cleaned = afterThink
      .replace(/<\/?think>/g, "")
      .replace(/<\/?uncertain>/g, "")
      .trim();
    return { innerThought: null, response: cleaned, uncertain, truncated: false };
  }

  const innerThought = raw.slice(openIdx + 7, closeIdx).trim();
  const response = afterThink.replace(/^\s*\n/, "").trimStart();

  return { innerThought, response, uncertain, truncated: false };
}
