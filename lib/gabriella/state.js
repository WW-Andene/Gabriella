...
// for warmth). Also takes optional `context` for organism dimensions —
// things like pragmatic weight, gap since last turn, whether this is
// re-entry.

export function foldFeltState(state, feltState, context = {}) {
  ...
  const weight = Number(context.pragmaticWeight ?? 0.3);   // 0..1, how substantive the moment
  const gapMs  = Number(context.gapSinceLastTurnMs ?? 0);  // ms since last exchange
  const isReentry = Boolean(context.isReentry);

  // ─── Emotional dimensions (unchanged logic) ─────────────────────────────
  ...
  // Energy — depleted by intense turns, restored between sessions.
  // A heavy (high-weight) turn drains more than a light one. Gaps between
  // turns restore (already handled by decayState pulling toward 0.75 rest),
  // but this fold takes a direct HIT from the turn itself.
  const energyCost =
    weight >= 0.7 ? 0.15 :   // heavy turn — real work
    weight >= 0.4 ? 0.07 :   // moderate turn
    weight >= 0.2 ? 0.03 :   // casual turn
                    0.01;    // phatic — almost free

  // Cost is deducted from current energy, not a target. Decay-toward-rest
  // handles recovery between turns.
  const energyAfterTurn = clamp((state.energy ?? 0.75) - energyCost);

  // Attention — sharpens on substantive turns (fresh input), decays fast
  // otherwise. A big-substrate turn pulls attention high; phatic ones
  // leave it to drift.
  const attentionTarget =
    weight >= 0.7 ? 0.85 :
    weight >= 0.4 ? 0.70 :
    weight >= 0.2 ? 0.55 :
                    state.attention;   // no change for low-weight

  // Social comfort — builds slowly with good turns, drops on cold/irritation,
  // resets slightly on re-entry after long gaps.
  // Good turn = positive charge present, low irritation, no 'cold' signals.
  const goodTurnSignal = warmSignal != null && warmSignal > 0.5 ? 0.05 : 0;
  const coldTurnSignal = warmSignal != null && warmSignal < 0.3 ? -0.05 : 0;
  const irritationCost = (irritTarget > 0.5) ? -0.08 : 0;
  // Re-entry after a long gap (>24h) softens socialComfort slightly —
  // not reset, just a small step back toward neutral.
  const reentryAdjust = isReentry && gapMs > 24 * 60 * 60 * 1000 ? -0.05 : 0;

  const socialDelta = goodTurnSignal + coldTurnSignal + irritationCost + reentryAdjust;
  const socialTarget = clamp((state.socialComfort ?? 0.5) + socialDelta);

  const mix = (a, b, w) => clamp(a * (1 - w) + b * w);

  // Pragmatic-weight-aware mix boost. A heavy turn SHOULD pull state
  // toward the moment's reading faster than a casual one — otherwise the
  // organism is observably sluggish. At weight 0.3 (default) we use the
  // original mix factors; at weight 0.8 we lift them up to +0.2 so the
  // felt-state dominates. Capped at 1.0.
  const boost = Math.max(0, Math.min(0.2, (weight - 0.3) * 0.4));
  const b = (base) => Math.min(1, base + boost);

  return {
    ...state,
    // Emotional — heavy turns swing openness / alertness / care more
    // decisively than casual ones.
    openness:   mix(state.openness,   openTarget,  b(0.45)),
    alertness:  mix(state.alertness,  alertTarget, b(0.5)),
    care:       mix(state.care,       careTarget,  b(warmSignal != null ? 0.5 : 0.2)),
    irritation: mix(state.irritation, irritTarget, b(0.4)),
    // Warmth is deliberately slow — the relationship-level dim. Leave it.
    warmth:     mix(state.warmth,     careTarget,  warmSignal != null ? 0.1 : 0.02),
    // Organism — energy is set directly (not mixed), others are blended.
    // Attention also lifts on heavy turns.
    energy:        energyAfterTurn,
    attention:     mix(state.attention ?? 0.6,     attentionTarget, b(0.6)),
    socialComfort: socialTarget,   // already represents the target — don't mix again
    updatedAt:  Date.now(),
  };
}
...
function describe(dim, value) {
  const bands = {
    ...
    energy: [
      [0.25, "depleted — less to give this turn"],
      [0.45, "a little tired, working harder than usual"],
      [0.70, null],              // ordinary energy — don't mention
      [1.01, "unusually rested and present"],
    ],
    attention: [
      [0.35, "a bit drifty, less sharp"],
      [0.55, null],              // ordinary attention — don't mention
      [0.75, "tracking closely"],
      [1.01, "locked in"],
    ],
    socialComfort: [
      [0.30, "still a little careful with them — not fully at ease"],
      [0.50, null],              // neutral — don't mention
      [0.70, "comfortable with them, less filter"],
      [1.01, "fully at ease — you're not managing this relationship"],
    ],
  };
...
export function getStateBlock(state) {
  ...
  const parts = [
    ...
    describe("energy",        state.energy),
    describe("attention",     state.attention),
    describe("socialComfort", state.socialComfort),
  ].filter(Boolean);
...
async function llmNudge({ state, messages, reply }) {
  ...
  const prompt = `Given this recent exchange, suggest a small delta to Gabriella's organism state vector. Return ONLY JSON.
...
- openness: lowered when she pulled back, raised when they earned more of her
- care: about THIS exchange; warmth: about the relationship overall
- irritation: friction; alertness: how closely she's tracking

Organism dimensions:
- energy: lowered by draining turns (real work, heavy emotional load,
  arguments); raised only slightly by fresh engagement
- attention: quality of tracking — sharper on interesting/substantive,
  drifts on phatic
- socialComfort: builds with good connection, drops with coldness or
  being treated as utility

Return: {"openness":<d>,"alertness":<d>,"care":<d>,"irritation":<d>,"warmth":<d>,"energy":<d>,"attention":<d>,"socialComfort":<d>,"note":"<10-word reason>"}`;
  ...
  try {
    ...
    return {
      ...
      energy:        apply(state.energy,        delta.energy),
      attention:     apply(state.attention,     delta.attention),
      socialComfort: apply(state.socialComfort, delta.socialComfort),
      updatedAt:     Date.now(),
    };
  } catch {
    return state;
...
export async function updateState(redis, userId, {
  ...
  pragmaticWeight,       // 0..1 — how substantive this turn was
  gapSinceLastTurnMs,    // ms since last exchange (for re-entry detection)
  isReentry,             // boolean — is this a first-turn-of-new-session
}) {
  try {
    const current = await loadState(redis, userId);
    // Context forwarded to foldFeltState so energy/attention/socialComfort
