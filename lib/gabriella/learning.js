...
import { buildDpoBundle, buildKtoBundle } from "./preferences.js";
import { readEnsembleLabels, ensembleLabelsToKtoExamples } from "./ensembleJudge.js";
import { heuristicCheck }               from "./metacognition.js";
import { loadFinetuneConfig }           from "./finetuneConfig.js";
import {
  uploadDataset   as fwUploadDataset,
...
// ─── Filtering ────────────────────────────────────────────────────────────────
// Training-data filter must be AT LEAST AS STRICT as the runtime gauntlet,
// otherwise we train the model on phrases the gauntlet rejects — the model
// learns contradictory signals. Reuses the runtime heuristic check imported
// from metacognition.js so the two stay in lockstep.
...
export function isValidExample(entry) {
  ...
  const check = heuristicCheck(entry.response);
  if (!check.authentic) return false;
  return true;
}
...
// Uploads the JSONL as a Fireworks dataset. Uses the current API flow
// verified against docs.fireworks.ai:
//
//   1. POST /v1/accounts/{acct}/datasets
//   2. POST .../{dataset_id}:upload            (multipart, ≤150MB)
//   3. POST .../{dataset_id}:validateUpload
//
// Returns { datasetId } on success. Errors propagate with the exact URL
// and response body so failures are diagnosable.
...
export async function uploadToFireworks(jsonl, apiKey, { filename, accountId }) {
  ...
  const exampleCount = Math.max(1, jsonl.split("\n").filter(l => l.trim()).length);

  const result = await fwUploadDataset({
    apiKey, accountId, jsonl, filename, exampleCount,
  });
  ...
  return {
    ...
    bytes:     result.bytes,
    filename,
    flow:      result.flow,  // "multipart" for our typical sizes
  };
}
...
// ─── Read back an archived bundle ─────────────────────────────────────────────
// Reconstructs a JSONL bundle stored by archiveToUpstash. Handles both the
// single-value and chunked formats. If given a key without a timestamp
// suffix, looks up the latest archive of that kind by scanning.

export async function readArchivedBundle(redis, archiveKey) {
  // Direct single-value case.
  const direct = await redis.get(archiveKey);
  if (direct) return { jsonl: direct, meta: null, chunks: 1 };

  // Chunked case — look for :meta.
  const metaRaw = await redis.get(`${archiveKey}:meta`);
  if (metaRaw) {
    const meta = typeof metaRaw === "string" ? JSON.parse(metaRaw) : metaRaw;
    const parts = [];
    for (let i = 0; i < meta.chunks; i++) {
      const part = await redis.get(`${archiveKey}:${i}`);
      if (part == null) throw new Error(`chunk ${i} missing from archive`);
      parts.push(part);
    }
    return { jsonl: parts.join(""), meta, chunks: meta.chunks };
  }

  throw new Error(`archive not found at key: ${archiveKey}`);
}

// Find the most recent archive of a given kind for a user. Tries KEYS
// first (simple, works on Upstash for small keyspaces), falls back to
// SCAN for larger sets.
export async function findLatestArchiveKey(redis, userId, kind = "bootstrap") {
  const pattern = `${userId}:learning:archive:${kind}:*`;
  let keys = [];

  // Primary: KEYS command. Upstash allows this for keyspaces under ~100k
  // without performance issues, which is our case here.
  try {
    const direct = await redis.keys(pattern);
    if (Array.isArray(direct) && direct.length > 0) keys = direct;
  } catch {
    // ignore, fall through to scan
  }

  // Fallback: SCAN if KEYS didn't return anything.
  if (keys.length === 0) {
    try {
      let cursor = 0;
      do {
        const res = await redis.scan(cursor, { match: pattern, count: 500 });
        const [next, found] = Array.isArray(res) ? res : [res.cursor, res.keys];
        cursor = Number(next);
  ...
  // Filter out chunk and meta suffixes — want top-level bundle keys only.
  // Top-level keys have 5 colon-delimited segments:
  //   userId:learning:archive:kind:timestamp
  // Chunk/meta keys have 6:
  //   userId:learning:archive:kind:timestamp:0
  //   userId:learning:archive:kind:timestamp:meta
  // The old regex /(:\d+$)|(:meta$)/ incorrectly stripped top-level keys
  // because the timestamp itself is all digits.
  const top = keys.filter(k => k.split(":").length === 5);
  if (top.length === 0) return null;

  // Sort by trailing timestamp, newest first.
  top.sort((a, b) => {
...
// List all archive keys for a user across kinds — useful for diagnostics.
export async function listArchives(redis, userId) {
  const pattern = `${userId}:learning:archive:*`;
  let keys = [];
  try {
    const direct = await redis.keys(pattern);
    if (Array.isArray(direct)) keys = direct;
  } catch {}
  // Filter out chunk and meta suffixes — keep only top-level (5-segment) keys.
  return keys.filter(k => k.split(":").length === 5).sort();
}
...
export async function pushLearningBundle(redis, userId, {
  ...
  // KTO bundle — single-label training (thumbs-up / thumbs-down).
  // Three sources combined:
  //   1. DPO preference pairs — chosen → up, rejected → down
  //   2. Ensemble-judged turns — every gauntlet-passing response where
  //      ≥2 of 3 family judges agreed on up/down. This is where the bulk
  //      of new training signal comes from; the gauntlet's binary pass/
  //      fail doesn't separate "fine but flat" from "textbook Gabriella,"
  //      but the ensemble does.
  //   3. Any other single-label sources passed via extraExamples.
  const ensembleLabels = await readEnsembleLabels(redis, userId, {
    sinceTimestamp: effectiveSince,
  });
  const ensembleExamples = ensembleLabelsToKtoExamples(ensembleLabels);

  const ktoBundle = await buildKtoBundle(redis, userId, {
    sinceTimestamp: effectiveSince,
    extraExamples:  ensembleExamples,
  });
  const ktoFilename = `gabriella-kto-${datestamp}.jsonl`;
  // Lower threshold than DPO — KTO tolerates smaller, imbalanced sets.
  const ktoReady = ktoBundle.stats.examples >= 20;
  ...
  if (env.FIREWORKS_API_KEY) {
    ...
    // Fireworks — KTO (single-label; lower threshold, broader data source)
    if (ktoReady) {
      try {
        const ktoUpload = await uploadToFireworks(ktoBundle.jsonl, env.FIREWORKS_API_KEY, {
          filename:  ktoFilename,
          accountId: env.FIREWORKS_ACCOUNT_ID,
        });
        uploads.push({
          ...ktoUpload,
          kind: "kto",
          exampleCount: ktoBundle.stats.examples,
          thumbsUp:     ktoBundle.stats.thumbsUp,
          thumbsDown:   ktoBundle.stats.thumbsDown,
          ratio:        ktoBundle.stats.ratio,
        });
      } catch (err) {
        errors.push({ provider: "fireworks-kto", error: err.message });
      }
    }
  }
  ...
  if (ktoReady) {
    try {
      uploads.push(await archiveToUpstash(redis, userId, ktoBundle.jsonl, {
        kind:     "kto",
        filename: ktoFilename,
      }));
    } catch (err) {
      errors.push({ provider: "upstash-archive-kto", error: err.message });
    }
  }
...
export async function maybeTriggerFineTune(redis, userId, pushResult) {
  ...
  // Apply runtime-tunable config (defaults ← env ← upstash overrides).
  const { config: ft } = await loadFinetuneConfig(redis);
  const displayName = `${ft.displayNamePrefix}-${new Date().toISOString().slice(0, 10)}`;
