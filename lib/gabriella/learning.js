...
import {
  uploadDataset   as fwUploadDataset,   // current flow (signed URL)
  uploadFile      as fwUploadFile,      // legacy /files endpoint (fallback)
...
// Uploads the JSONL as a dataset that SFT jobs can consume. Uses the
// current dataset-with-signed-URL flow. If that fails with 404/not-found
// (older accounts), falls back to the legacy /files + /datasets flow.
...
export async function uploadToFireworks(jsonl, apiKey, { filename, accountId }) {
  ...
  // Primary path: current flow (signed-URL upload, no /files endpoint).
  try {
    ...
    return {
      ...
      bytes:     result.bytes,
      filename,
      flow:      result.flow, // "signed-url"
    };
  } catch (primaryErr) {
    const is404 = primaryErr.status === 404 ||
                  /"code"\s*:\s*5|not\s*found/i.test(primaryErr.body || primaryErr.message || "");
    if (!is404) {
      // Real error (auth, quota, etc.) — don't mask by falling back.
      throw primaryErr;
    }
    console.warn(`Fireworks signed-URL upload 404'd, trying legacy /files path: ${primaryErr.message?.slice(0, 200)}`);
  }

  // Fallback: legacy /files path.
  try {
    const { fileId } = await fwUploadFile({ apiKey, accountId, jsonl, filename });
    if (!fileId) throw new Error("Legacy file upload returned no file id");
    ...
    // Both flows failed. Throw a combined error that tells the user
    // exactly what was tried and why neither worked.
    throw new Error(
      `Fireworks upload failed on both flows.\n` +
      `  - Signed-URL flow: primary endpoint returned 404\n` +
      `  - Legacy /files flow: ${fallbackErr.message}\n\n` +
      `Most likely: FIREWORKS_ACCOUNT_ID="${accountId}" doesn't match any account the API key can access.\n` +
      `Check the slug at https://fireworks.ai/account/profile and update it in .env.local + Vercel.`,
    );
  }
}
...
// Find the most recent archive of a given kind for a user. Uses SCAN so
// it handles large keyspaces without blocking Redis.
export async function findLatestArchiveKey(redis, userId, kind = "bootstrap") {
  const pattern = `${userId}:learning:archive:${kind}:*`;
  let cursor = 0;
  const keys = [];
  // Upstash SDK supports scan differently across versions; use pagination.
  do {
  ...
  const top = keys.filter(k => !/(:\d+$)|(:meta$)/.test(k));
