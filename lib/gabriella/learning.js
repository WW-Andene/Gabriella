...
export async function pushLearningBundle(redis, userId, {
  ...
  // Doubles the example count from the DPO pair store by emitting both
  // sides as independent labeled examples, and also unlocks single-label
  // sources (self-eval failures → thumbs-down) via the extraExamples hook.
  // Works when DPO pair volume is too low. Per Ethayarajh et al. 2024,
  // KTO matches or beats DPO on persona-consistency objectives — which
  // is exactly what Gabriella is optimizing for.
