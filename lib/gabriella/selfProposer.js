...
export async function proposeSelfDeltas(redis, userId, {
  ...
  const deltas = await withBreaker(redis, "selfProposer", async () => {
    ...
    if (parsed && Array.isArray(parsed.deltas)) {
      return parsed.deltas.slice(0, MAX_DELTAS_PER_TURN);
    }
    return [];
  }, { fallback: [], failureThreshold: 5, coolDownMs: 5 * 60_000 });
