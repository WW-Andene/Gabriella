...
// DPO requires preference PAIRS (same prompt, two completions, labeled).
// KTO requires only single labeled completions: this prompt → this output
// was GOOD, or this prompt → this output was BAD. No pair needed.
//
// Why this matters under our constraints: the gauntlet/DPO pipeline only
// captures pairs when (a) the gauntlet rejects, AND (b) the retry passes,
// AND (c) the retry is different. That's a narrow gate. The self-eval
// harness, by contrast, produces single labeled outputs by the thousand —
// every failure is a "bad" example with its exact context, every passing
// turn is a "good" example. With KTO we can fine-tune on ALL of it.
//
// Empirically KTO matches or beats DPO on persona tasks and tolerates
// imbalanced thumbs-up/thumbs-down distributions well (Ethayarajh et al.
// 2024). Fireworks and Together both accept KTO-format datasets for LoRA.
//
// KTO JSONL shape (matches Fireworks + Together current spec):
//   { "messages": [{role, content}, ...], "label": true }    // thumbs-up
//   { "messages": [{role, content}, ...], "label": false }   // thumbs-down

const KTO_SYSTEM = DPO_SYSTEM;   // same voice constraint as DPO bundle

export function buildKtoExample({ context, output, label }) {
  if (!context || !output || typeof label !== "boolean") return null;
  return {
    messages: [
      { role: "system",    content: KTO_SYSTEM },
      ...context,
...
// Build a KTO bundle from the preference-pair store by emitting BOTH sides:
// the chosen output as thumbs-up, the rejected output as thumbs-down, same
// context. This 2x's the example count from the same underlying data and
// also unlocks single-label sources (self-eval failures) to be dropped in
// via the same writer.
export async function buildKtoBundle(redis, userId, {
  sinceTimestamp = null,
  extraExamples = [],    // [{ context, output, label }] — e.g. from self-eval directly
} = {}) {
  const pairs = await readPreferencePairs(redis, userId, { sinceTimestamp });
  const examples = [];

  for (const p of pairs) {
    const up = buildKtoExample({
      context: p.context, output: p.chosen,   label: true,
    });
    const down = buildKtoExample({
      context: p.context, output: p.rejected, label: false,
    });
    if (up)   examples.push(up);
    if (down) examples.push(down);
  }

  for (const e of extraExamples) {
    const built = buildKtoExample(e);
    if (built) examples.push(built);
  }

  // KTO works best with a reasonable thumbs-up/down balance (ideally 50/50
  // ± 25%). Log the ratio so the training pipeline can decide whether to
  // hold off or rebalance.
  const up   = examples.filter(e => e.label === true).length;
  const down = examples.filter(e => e.label === false).length;

  const jsonl = examples.map(e => JSON.stringify(e)).join("\n");

  return {
    stats: {
      pairsSource:  pairs.length,
      examples:     examples.length,
      thumbsUp:     up,
      thumbsDown:   down,
      ratio:        examples.length > 0 ? +(up / examples.length).toFixed(2) : null,
      firstAt:      pairs.length ? Math.min(...pairs.map(p => p.t || 0)) : null,
      lastAt:       pairs.length ? Math.max(...pairs.map(p => p.t || 0)) : null,
    },
    jsonl,
