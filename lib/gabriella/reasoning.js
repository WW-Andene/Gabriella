...
//
// RETIRED (Phase C) — this function is no longer called from the per-turn
// pipeline. The speaker's hidden <think> block (monologue.js) does the
// same work end-to-end inside a single LLM call, which the audit found
// was substantively duplicative with deliberation's output. Keeping the
// function exported so bootstrap / self-eval / experimental callers that
// specifically want an OUT-OF-BAND thinking pass can still invoke it,
// but the live conversation path does not. See turn.js — `deliberation`
// is hard-coded to null when calling speak().
