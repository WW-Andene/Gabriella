...
import { detectWit, shouldSuppressWit } from "./humor.js";
...
export async function runTurn({
  ...
  // If silence policy fired, also override length + temperature.
  let taggedFeltState = { ...feltState, _mood: currentMood };
  if (silenceMoment) {
    taggedFeltState = applySilenceOverride(taggedFeltState, silenceMoment);
  }

  // Wit detection — high-precision regex flags moments where dryness /
  // playful counter / deadpan extension is the right move. Suppressed
  // automatically on heavy moments (pragmatic weight >= 0.55) and when
  // silence policy fired. Attaches to feltState as _wit so the speaker
  // prompt's wit block renders.
  const witHit = !silenceMoment ? detectWit(lastUserForSilence) : null;
