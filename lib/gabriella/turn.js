...
import { computeTurnShape, explainShape } from "./turnShape.js";
...
export async function runTurn({
  ...
  // ── 0. Turn shape — ONE routing decision from ALL read-layer signals ──
  // Replaces the scattered per-feature checks that used to decide
  // cognition path, gauntlet mode, rollout enablement, etc. Computed
  // once, consumed everywhere downstream.
  const turnShape = await computeTurnShape({
    redis, userId, pragmatics, messages: recentMessages,
  }).catch(() => ({
    cognitionPath: "triple", gauntletMode: "full",
    rolloutPolicy: { enabled: false, threshold: 0.5, depth: 1 },
    adaptSignals: { styleOutcomes: null, stance: "unknown", heatmap: null, rolloutConfidence: null },
    activeTension: null, markers: {}, weight: pragmatics?.weight ?? 0.3, stance: "unknown",
  }));

  // ── 1. Cognition ────────────────────────────────────────────────────────
  // Routing comes from turnShape.cognitionPath. Priors (person +
  // narrative + reasoning trace) are threaded into BOTH paths so
  // cognition is continuous instead of cold-starting every turn.
  if (turnShape.cognitionPath === "unified") {
    logWarn("turn", "cognition path = unified (pool/flag)", { pool: poolStats(), shape: explainShape(turnShape) }).catch(() => {});
  }
  ...
  const cognitionContext = {
    ...
    person:    personModel,
    narrative,
    // Active dialectical tension — if her prior positions clash with
    // what this moment is asking, raise it to cognition so she speaks
    // FROM the tension instead of pretending coherence.
    activeTension: turnShape.activeTension,
  };

  const cognition = (turnShape.cognitionPath === "triple")
  ...
  const gauntletResult = needsGauntlet
    ? await runGauntlet(candidate, recentMessages, withheld, questionEval, activeAgenda, activeThreshold, { mode: turnShape.gauntletMode })
  ...
  if (!gauntletResult.pass) {
    ...
    // Retry runs at "full" gauntlet regardless of original mode — a
    // rejection is already evidence something's off; don't risk a
    // relaxed re-check.
    const retryGauntlet  = retryHeuristic.authentic
      ? await runGauntlet(retry, recentMessages, withheld, questionEval, activeAgenda, activeThreshold, { mode: "full" })
  ...
  return {
    ...
    turnKnobs,
    substrateDelta,
    // Routing trace — what turnShape decided for this turn. Logged
    // so the per-turn decision is auditable without re-running.
    turnShape: {
      cognitionPath: turnShape.cognitionPath,
      gauntletMode:  turnShape.gauntletMode,
      rolloutPolicy: turnShape.rolloutPolicy,
      stance:        turnShape.stance,
      activeTension: turnShape.activeTension ? {
        topic:   turnShape.activeTension.topic,
        verdict: turnShape.activeTension.verdict,
      } : null,
      explain: explainShape(turnShape),
    },
  };
}
