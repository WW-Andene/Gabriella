...
export async function uploadDatasetMultipart({ apiKey, accountId, datasetId, jsonl, filename }) {
  ...
  // Tolerate "already uploaded" — same data has already been accepted for
  // this dataset. Treat as success so a retried push doesn't throw.
  const body = await res.text().catch(() => "");
  const isAlreadyDone =
    /"code"\s*:\s*9/.test(body) ||
    /already\s*(updated|uploaded|exists|ready)/i.test(body);

  if (isAlreadyDone) {
    return { uploaded: true, note: "dataset already had content" };
  }

  const err = new Error(
    `Fireworks dataset upload (${datasetId}:upload) failed (${res.status}) at ${res.url}\n  body: ${body.slice(0, 600)}`,
  );
  err.status = res.status;
  err.body   = body;
  throw err;
...
// ─── 3. Validate upload ──────────────────────────────────────────────────────
// Note: on many accounts, the :upload endpoint auto-validates and sets the
// dataset to READY immediately. A subsequent :validateUpload then returns
// gRPC code 9 (FAILED_PRECONDITION) with "already updated" — which is success
// from our perspective. We treat the following as a no-op success:
//   - HTTP 200 (validated now)
//   - HTTP 409 (already validated, legacy shape)
//   - HTTP 400/412 with code 9 or "already" in body (new shape)
...
export async function validateDatasetUpload({ apiKey, accountId, datasetId }) {
  ...
  const body = await res.text().catch(() => "");
  // Fireworks returns a gRPC-style envelope: {"code": 9, "message": "...", ...}
  const isAlreadyDone =
    /"code"\s*:\s*9/.test(body) ||
    /already\s*(updated|validated|exists|ready)/i.test(body);

  if (isAlreadyDone) {
    return { validated: true, note: "dataset was auto-validated on upload" };
  }
...
export async function uploadDataset({ apiKey, accountId, jsonl, filename, exampleCount }) {
  ...
  await createDatasetShell({
    apiKey, accountId, datasetId, displayName, exampleCount,
  });

  // 2. Upload — multipart when small, signed URL when large.
  let flow;
  if (jsonl.length <= MULTIPART_LIMIT_BYTES) {
    await uploadDatasetMultipart({
      apiKey, accountId, datasetId, jsonl,
      filename: filename || `${datasetId}.jsonl`,
    });
    flow = "multipart";
  } else {
    const { signedUrl } = await getDatasetUploadEndpoint({
      apiKey, accountId, datasetId,
      filename: filename || `${datasetId}.jsonl`,
      size:     jsonl.length,
    });
    await uploadToSignedUrl({ signedUrl, jsonl });
    flow = "signed-url";
  }
  ...
  return { datasetId, bytes: jsonl.length, flow };
}
...
// ─── Back-compat shims ────────────────────────────────────────────────────────
// Older call-sites imported these names. Keep them pointing at the current
// implementations so nothing needs touching outside this file.

export const uploadFile      = uploadDatasetMultipart;   // legacy name
export const createDataset   = createDatasetShell;       // legacy name
