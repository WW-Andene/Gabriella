...
const REWRITE_COOLDOWN_MS = 6 * 60 * 60 * 1000;   // 6 hours
const REWRITE_MIN_NEW_TURNS = 10;
...
export async function rewriteNarrative(redis, userId, {
  ...
  try {
    ...
    if (!force && age < REWRITE_COOLDOWN_MS && newSinceLastRewrite < REWRITE_MIN_NEW_TURNS) {
...
Write the new narrative. Output ONLY the four-section narrative, nothing else.`;
    ...
    const nextMeta = { lastRewriteAt: Date.now(), lastRewriteTurnCount: turnCount };
