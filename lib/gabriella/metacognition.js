...
// Flags now persist as a rolling LIST rather than a single overwriting
// key. This lets us detect recurrence — if the same issue fires three
// turns running, that's a different (and louder) signal than a one-off
// miss. Latest-first, capped at FLAG_WINDOW entries.

const FLAG_KEY    = (u) => `${u}:metacognitionFlags`;
const FLAG_WINDOW = 12;

async function storeFlag(redis, userId, data) {
  try {
    const entry = JSON.stringify({ ...data, timestamp: Date.now() });
    await redis.lpush(FLAG_KEY(userId), entry);
    await redis.ltrim(FLAG_KEY(userId), 0, FLAG_WINDOW - 1);
    // Keep the old single-key store for backward-compat in case any
    // external debug tooling is still reading it.
    await redis.set(
...
// Classify a flag's "issue" into a stable category so recurrence detection
// doesn't get fooled by slight text differences ("bullet points" vs
// "Response uses bullet point formatting").
function classifyIssue(issue) {
  if (!issue) return "unknown";
  const s = String(issue).toLowerCase();
  if (/bullet|numbered list/.test(s))           return "list_formatting";
  if (/starts with .?i./.test(s))                return "opens_with_i";
  if (/wrap-?up|summary|validation question/.test(s)) return "summary_ending";
  if (/banned phrase|recently-learned tell/.test(s))  return "banned_phrase";
  if (/over-explain|explaining a feeling/.test(s))    return "over_explain";
  if (/therapist|therapy/.test(s))                   return "therapist_voice";
  if (/customer service|softener/.test(s))            return "service_voice";
  if (/reach(ing)? for warmth|performing/.test(s))    return "false_warmth";
  if (/soft landing/.test(s))                        return "soft_landing";
  if (/chatbot opener/.test(s))                      return "chatbot_opener";
  return "other";
}

async function loadFlags(redis, userId) {
  try {
    const raw = await redis.lrange(FLAG_KEY(userId), 0, FLAG_WINDOW - 1);
...
export async function getMetacognitionBlock(redis, userId) {
  const entries = await loadFlags(redis, userId);
  if (entries.length === 0) {
    // Fall back to the legacy single-key path for users who haven't
    // generated any new-style entries yet.
    const raw = await redis.get(`${userId}:metacognitionFlag`).catch(() => null);
    if (!raw) return "";
    const flag = typeof raw === "string" ? JSON.parse(raw) : raw;
    const age  = Date.now() - flag.timestamp;
    if (age > 2 * 60 * 60 * 1000) return "";
    return buildSingleFlagBlock(flag);
  }

  // Only flags younger than 2h count toward the block.
  const fresh = entries.filter(f => Date.now() - (f.timestamp || 0) < 2 * 60 * 60 * 1000);
  if (fresh.length === 0) return "";

  // Count categories across the fresh window.
  const counts = {};
  for (const f of fresh) {
    const cat = classifyIssue(f.issue);
  ...
  // Recurring pattern (same category 3+ times in the fresh window) gets
  // a louder block — not just "last response was flagged" but "this has
  // been happening".
  if (topCount >= 3) {
    return buildRecurringBlock(topCat, topCount, latest);
  }

  // Otherwise: the one-off correction block, same as before.
  return buildSingleFlagBlock(latest);
}

function buildSingleFlagBlock(flag) {
...
const RECURRING_GUIDANCE = {
  list_formatting:   "You keep reaching for structured formatting (bullets, numbered lists). That's a reflex. Whatever was making you want to structure — resist it. Prose, not outlines.",
  opens_with_i:      "You keep opening with 'I'. Each time it announces you from outside the thought instead of speaking from inside it. Start with what landed, not with yourself.",
  summary_ending:    "You keep closing on a validation question. Your last sentence is where you ran out of true things, not where you checked if they followed. Stop before the question.",
  banned_phrase:     "You keep reaching for phrases that aren't you — therapy-speak, customer-service softeners. Slow down before the reflex finishes. They're not yours.",
  over_explain:      "You keep explaining feelings instead of having them. The texture is in the specific, not the meta-observation about the specific.",
  therapist_voice:   "You keep sliding into therapist voice — validating instead of responding, asking what makes them ask instead of answering. Meet them with you, not with technique.",
  service_voice:     "You keep using customer-service softeners. That's the wrong register for this relationship. Drop the buffers.",
  false_warmth:      "You keep reaching for warmth when cool precision would be truer. If you feel reserved, be reserved. Performed warmth reads as hollow.",
  soft_landing:      "You keep adding a question at the end as a soft landing when no real curiosity is there. Let the response end on the last true thing.",
  chatbot_opener:    "You keep opening with 'Here's a/the...' — a chatbot shape, not yours. Start with the thought, not a frame for the thought.",
  other:             "A specific reflex has been catching you more than once. The filter is flagging it repeatedly. Whatever it is — notice you're about to do it, and choose otherwise.",
  unknown:           "Multiple flags lately. The details vary but something has been slipping.",
};

function buildRecurringBlock(cat, count, latest) {
  const guide = RECURRING_GUIDANCE[cat] || RECURRING_GUIDANCE.other;
  return `# METACOGNITION — RECURRING PATTERN
This has fired ${count} times in the last few turns. It's not a one-off — it's a reflex you've been repeating.

${guide}

Most recent excerpt: "${latest.excerpt}"

Notice the pull toward the old shape BEFORE you finish the sentence this time. That's the only way the pattern breaks.`;
}

// Exposed so tests can verify the categorizer and so external tooling
// can inspect the fresh flag window without duplicating the load logic.
export { classifyIssue, loadFlags as loadMetacognitionFlags };
